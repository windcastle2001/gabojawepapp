...
function toCategoryLabel(category: string) {
  ...
  if (category.includes('공원') || category.includes('관광') || category.includes('문화') || category.includes('산책')) return '산책';
  return '액티비티';
}
...
function StarRating({ value }: { value: number }) {
  return <span aria-label={`평점 ${value}`}>{[1, 2, 3, 4, 5].map((star) => <span key={star} className={star <= Math.round(value) ? 'text-yellow-400' : 'text-gray-200'}>★</span>)}</span>;
}
  ...
  if (window.kakao?.maps?.services?.Places && window.kakao.maps.services.Status) {
    return new Promise<KakaoSearchResult[]>((resolve, reject) => {
      ...
      service.keywordSearch(query, (data, status) => {
        ...
        if (status === window.kakao!.maps.services!.Status.ZERO_RESULT) resolve([]);
        else reject(new Error('카카오 장소 검색에 실패했습니다.'));
      });
    });
  }

  const response = await fetch(`/api/kakao/search?q=${encodeURIComponent(query)}`, { cache: 'no-store' });
  const data = (await response.json()) as { results?: KakaoSearchResult[]; error?: string };
  if (!response.ok) throw new Error(data.error ?? '카카오 장소 검색에 실패했습니다.');
  return data.results ?? [];
}
      ...
      <div className="w-full max-w-lg rounded-t-3xl bg-background p-6 shadow-2xl md:rounded-3xl">
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">장소를 직접 등록하거나 검색 결과를 보정할 수 있어요.</p>
          </div>
        ...
        <div className="space-y-4">
          <input value={title} onChange={(event) => setTitle(event.target.value)} className="w-full rounded-2xl border border-border bg-card px-4 py-3 text-sm outline-none focus:border-brand" placeholder="장소 이름" />
          ...
          <input value={address} onChange={(event) => setAddress(event.target.value)} className="w-full rounded-2xl border border-border bg-card px-4 py-3 text-sm outline-none focus:border-brand" placeholder="주소" />
          <textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} className="w-full rounded-2xl border border-border bg-card px-4 py-3 text-sm outline-none focus:border-brand" placeholder="추천 이유나 메모" />
          <label className="flex items-start gap-3 rounded-2xl border border-border bg-card px-4 py-3">
            ...
            <span className="text-sm font-semibold text-foreground">내 위시리스트에도 같이 저장</span>
          </label>
          <button
            onClick={() => {
...
function SearchResultSheet({
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center">
      <div className="pointer-events-auto max-h-[55vh] w-full max-w-2xl overflow-y-auto rounded-t-[28px] bg-background px-5 pb-8 pt-4 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-bold text-foreground">검색 결과</p>
            <p className="mt-1 text-xs text-muted-foreground">카카오 장소 검색 결과에서 바로 저장하거나 제보할 수 있어요.</p>
          </div>
          ...
          {results.map((item) => (
            <div key={item.id} className="rounded-2xl border border-border bg-card px-4 py-3">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button onClick={() => onAddWish(item)} className="rounded-xl bg-brand/10 px-3 py-2 text-xs font-semibold text-brand">위시 추가</button>
                <button onClick={() => onReport(item)} className="rounded-xl bg-secondary/10 px-3 py-2 text-xs font-semibold text-secondary">맵에 제보</button>
                <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="rounded-xl bg-muted px-3 py-2 text-xs font-semibold text-muted-foreground">원본 보기</a>
              </div>
            </div>
          ))}
        </div>
...
function BottomSheet({
  ...
  const reviews =
    activeReviewFilter === '전체'
      ? place.reviews
      : place.reviews.filter((review) => (activeReviewFilter === '커플 리뷰' ? review.reviewType === 'couple' : review.reviewType === 'friends'));
      ...
      <div className="pointer-events-auto relative max-h-[72vh] w-full overflow-y-auto rounded-t-[28px] bg-background shadow-2xl animate-slide-up-sheet">
        <div className="px-5 pb-8 pt-5">
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            <button onClick={onClose} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground" aria-label="닫기">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="mb-4 space-y-2">
            {reviews.length ? reviews.map((review: PrototypeReview) => (
              <div key={review.id} className="rounded-2xl bg-card p-3">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  <span className={cn('rounded-full px-2 py-0.5 text-[10px] font-semibold', review.reviewType === 'couple' ? 'bg-brand/10 text-brand' : 'bg-secondary/10 text-secondary')}>
            ...
            )) : <div className="rounded-2xl bg-card px-4 py-4 text-xs text-muted-foreground">아직 표시할 리뷰가 없어요.</div>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => onAddWish(place)} className="rounded-2xl bg-brand/10 py-3 text-sm font-semibold text-brand">위시 추가</button>
            <a href={`https://map.kakao.com/link/map/${encodeURIComponent(place.title)},${place.lat},${place.lng}`} target="_blank" rel="noreferrer" className="rounded-2xl bg-brand py-3 text-center text-sm font-semibold text-brand-foreground shadow-md shadow-brand/25">카카오맵 열기</a>
          </div>
        </div>
      </div>
    </div>
...
export default function CommunityMapPage() {
  ...
  const filteredPlaces = useMemo(() => {
    return places.filter((place) => {
      ...
      const matchReview =
        activeReviewFilter === '전체' ||
        place.reviews.some((review) => (activeReviewFilter === '커플 리뷰' ? review.reviewType === 'couple' : review.reviewType === 'friends'));
      const query = searchQuery.trim().toLowerCase();
      const matchSearch = !query || place.title.toLowerCase().includes(query) || place.address.toLowerCase().includes(query);
      const matchNearby = !nearbyOnly || !userLocation || getDistanceKm(userLocation, { lat: place.lat, lng: place.lng }) <= NEARBY_RADIUS_KM;
  ...
  function handleNearMe() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setNearbyOnly(true);
        setToast('현재 위치 기준 3km 안쪽만 보여드릴게요.');
      },
  ...
  async function handleSearchSubmit() {
    ...
    if (trimmed.startsWith('http://') || trimmed.startsWith('https://')) {
      ...
      try {
        ...
        setReportDraft({
          ...
          category: toCategoryLabel(data.data.category ?? ''),
          address: data.data.address ?? '',
          lat: data.data.lat,
          lng: data.data.lng,
          note: data.data.summary ?? '',
    ...
    try {
      ...
      setToast(results.length ? '카카오 검색 결과를 불러왔어요.' : '검색 결과가 없어요.');
    } catch (error) {
  ...
  return (
    <div className="relative flex h-screen flex-col overflow-hidden">
      ...
      <div className="pointer-events-none relative z-10">
        <div className="pointer-events-auto px-4 pb-3 pt-12">
          <div className="mb-2 flex items-center gap-2 rounded-2xl border border-border bg-background px-4 py-3 shadow-md">
            ...
            <input type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); handleSearchSubmit(); } }} placeholder="장소 이름, 지역, 네이버/카카오맵 링크로 검색" className="flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground" aria-label="장소 검색" />
            <button onClick={handleSearchSubmit} className="rounded-xl bg-brand px-3 py-2 text-xs font-semibold text-brand-foreground">{isSearching ? '검색 중' : '검색'}</button>
          </div>

          <div className="scrollbar-hide flex gap-2 overflow-x-auto pb-1">
            {CATEGORIES.map((category) => (
              <button key={category} onClick={() => setActiveCategory(category)} className={cn('shrink-0 rounded-full border px-4 py-2 text-xs font-semibold shadow-sm transition-colors', activeCategory === category ? 'border-brand bg-brand text-brand-foreground' : 'border-border bg-background text-muted-foreground')} aria-pressed={activeCategory === category}>
                {category}
              </button>
            ))}
          </div>
          ...
          <div className="scrollbar-hide mt-2 flex gap-2 overflow-x-auto pb-1">
            {REVIEW_FILTERS.map((filter) => (
              <button key={filter} onClick={() => setActiveReviewFilter(filter)} className={cn('shrink-0 rounded-full border px-4 py-2 text-xs font-semibold shadow-sm transition-colors', activeReviewFilter === filter ? 'border-secondary bg-secondary text-secondary-foreground' : 'border-border bg-background text-muted-foreground')} aria-pressed={activeReviewFilter === filter}>
                {filter}
              </button>
            ))}
          </div>
        ...
        <div className="pointer-events-auto absolute right-4 top-14 flex flex-col gap-2">
          <button onClick={handleNearMe} className={cn('rounded-xl border bg-background px-3 py-2 text-xs font-semibold shadow-md transition-colors', nearbyOnly ? 'border-brand bg-brand/5 text-brand' : 'border-brand/20 text-brand hover:bg-brand/5')}>
            <span className="inline-flex items-center gap-1"><LocateFixed className="h-3.5 w-3.5" />{nearbyOnly ? '내 주변 해제' : '내 주변'}</span>
          </button>
          <button onClick={() => openReportForResult()} className="rounded-xl border border-secondary/20 bg-background px-3 py-2 text-xs font-semibold text-secondary shadow-md transition-colors hover:bg-secondary/5">
            <span className="inline-flex items-center gap-1"><MapPinPlus className="h-3.5 w-3.5" />제보하기</span>
          </button>
        </div>
      </div>
      ...
      {searchResults.length > 0 ? <SearchResultSheet results={searchResults} onAddWish={handleAddWish} onReport={(item) => openReportForResult(item)} onClose={() => setSearchResults([])} /> : null}
      {showReportModal ? <ReportModal initialValue={reportDraft} onClose={() => setShowReportModal(false)} onSubmit={handleReportSubmit} /> : null}
      {toast ? <div className="animate-slide-up fixed bottom-28 left-1/2 z-50 -translate-x-1/2 rounded-full bg-foreground px-4 py-2.5 text-xs font-medium text-background shadow-lg">{toast}</div> : null}
    </div>
  );
}
